...
export type ClubProgramActionState = {
  error: string | null;
  success: string | null;
};

async function requireClubContext() {
  ...
  return { supabase, userId: user.id, clubId: club.id };
}

export async function createProgramAction(
  _prevState: ClubProgramActionState,
  formData: FormData,
): Promise<ClubProgramActionState> {
  ...
  revalidatePath("/dashboard/club/programas");
  return { error: null, success: "Programa creado correctamente para el club." };
}
...
export async function assignProgramToStudentAction(
  ...
  // Verificar que el programa pertenece al club y está activo.
  ...
  await createNotification({
    ...
    title: "Inscripción a programa del club",
    message: `El club te inscribió en el programa "${programa.nombre}". Se generaron todas las clases del período.`,
  });
