...
import { AssignProgramForm } from "@/app/dashboard/profesor/programas/assign-program-form";
import { ProgramForm } from "@/app/dashboard/profesor/programas/program-form";
import {
...
export default async function ClubProgramasPage({ searchParams }: PageProps) {
  const club = await requireClub();
  const resolvedSearchParams = searchParams ? await searchParams : undefined;
  const activeTab = resolvedSearchParams?.tab === "membresias" ? "membresias" : "programas";

  const supabase = await createSupabaseServerClient();
  const [programasResult, inscripcionesResult, alumnosBookingResult] = await Promise.all([
    supabase
      .from("programs")
    ...
    supabase.from("bookings").select("alumno_id").eq("club_id", club.id).limit(200),
  ]);

  const programas = (programasResult.data ?? []) as ProgramRow[];
  const inscripciones = (inscripcionesResult.data ?? []) as StudentProgramRow[];
  const alumnosBookingRows = (alumnosBookingResult.data ?? []) as BookingAlumnoRow[];
  const uniqueAlumnoIds = Array.from(new Set(alumnosBookingRows.map((row) => row.alumno_id)));

  const { data: alumnosProfileData } = uniqueAlumnoIds.length
    ? await supabase.from("profiles").select("user_id, name").in("user_id", uniqueAlumnoIds)
    : { data: [] as Array<{ user_id: string; name: string }> };

  const alumnosMap = new Map<string, AlumnoOption>();
  for (const alumno of alumnosProfileData ?? []) {
    alumnosMap.set(alumno.user_id, {
      user_id: alumno.user_id,
      name: alumno.name?.trim() || "Alumno",
    });
  }
  for (const inscripcion of inscripciones) {
    if (!alumnosMap.has(inscripcion.alumno_id)) {
      alumnosMap.set(inscripcion.alumno_id, {
        user_id: inscripcion.alumno_id,
        name: "Alumno",
      });
    }
  }
  const alumnos = Array.from(alumnosMap.values()).sort((a, b) => a.name.localeCompare(b.name, "es"));
  const hasLoadError = Boolean(programasResult.error || inscripcionesResult.error || alumnosBookingResult.error);
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-[1600px] flex-col px-3 py-6 sm:px-4 sm:py-8">
      ...
      >
        <Link
          href="/dashboard/club/programas?tab=programas"
        ...
        <Link
          href="/dashboard/club/programas?tab=membresias"
          className="rounded-lg px-4 py-2 text-sm font-medium"
          style={
