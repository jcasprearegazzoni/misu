-- Soporte para precios por cancha individual.
-- cancha_id = NULL  → precio global para todas las canchas del deporte (comportamiento actual).
-- cancha_id = X     → precio específico para esa cancha (tiene prioridad sobre el global).

-- 1. Columna cancha_id en club_franjas_precio
alter table public.club_franjas_precio
  add column if not exists cancha_id integer references public.canchas(id) on delete cascade;

-- 2. Reemplazar unique constraint incluyendo cancha_id.
--    NULLS NOT DISTINCT: dos filas con cancha_id = NULL y mismo slot se tratan como conflicto.
alter table public.club_franjas_precio
  drop constraint if exists club_franjas_unique;

create unique index club_franjas_unique
  on public.club_franjas_precio (club_id, deporte, day_of_week, desde, duracion_minutos, cancha_id)
  nulls not distinct;

-- 3. Reescribir get_club_slots_disponibles: precio por cancha con fallback a global.
--    Se reemplaza el JOIN simple a club_franjas_precio por un JOIN LATERAL con DISTINCT ON,
--    que devuelve una fila por duracion priorizando el precio específico de la cancha.
...
as $$
  with ventanas as (
    ...
    join public.canchas c
     ...
     and c.activa = true
    -- Por cada cancha, seleccionar el mejor precio para cada duracion:
    -- cancha-específico tiene prioridad sobre global (cancha_id IS NULL).
    join lateral (
      select distinct on (fp2.duracion_minutos)
        fp2.duracion_minutos,
        fp2.precio,
        fp2.desde,
        fp2.hasta
      from public.club_franjas_precio fp2
      where fp2.club_id = d.club_id
        and fp2.deporte = d.deporte
        and fp2.day_of_week = d.day_of_week
        and fp2.duracion_minutos = any(d.duraciones)
        and (fp2.cancha_id = c.id or fp2.cancha_id is null)
      order by fp2.duracion_minutos, (fp2.cancha_id is not null) desc
    ) fp on true
