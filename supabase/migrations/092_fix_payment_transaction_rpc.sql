-- Corrige RPC create_payment_transaction para soportar student_program_id.
create or replace function public.create_payment_transaction(
  ...
  p_student_program_id bigint default null,
...
begin
  -- Valida que se especifique exactamente uno de student_package_id, student_program_id o reserva_cancha_id.
  if ((p_student_package_id is null)::int + (p_student_program_id is null)::int + (p_reserva_cancha_id is null)::int) != 2 then
    raise exception 'Debe especificarse exactamente uno de student_package_id, student_program_id o reserva_cancha_id';
  end if;
  ...
  where gateway_status = 'pending'
    ...
    and (
      ...
      or
      (p_student_program_id is not null and student_program_id = p_student_program_id)
      or
  ...
  insert into payment_transactions (
    ...
    student_package_id, reserva_cancha_id, student_program_id,
    profesor_id, club_id,
    payer_user_id, payer_email
  ) values (
    p_gateway, p_amount, p_currency,
    p_student_package_id, p_reserva_cancha_id, p_student_program_id,
...
grant execute on function public.create_payment_transaction(text, numeric, text, bigint, bigint, bigint, uuid, bigint, text) to authenticated;
