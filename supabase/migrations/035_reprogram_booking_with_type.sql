-- Extiende reprogram_booking_with_capacity para permitir cambio de modalidad
-- cuando el slot destino está vacío y el profesor especifica un nuevo tipo.
...
begin
  ...
  -- Validar que el nuevo slot exista en availability.
  ...
  -- Modalidad fija y cupo del slot destino (excluye el booking que se está moviendo).
  ...
  if v_existing_type is null then
    -- Slot destino vacío: el profesor puede elegir la modalidad.
    v_effective_type := coalesce(p_new_type, v_booking.type);
    v_active_count   := 0;
  else
    -- Slot destino con bookings: la modalidad está fija.
  ...
  -- Si había decisión pendiente para este booking, ya no aplica.
  ...
  -- El slot original puede haber quedado con un solo alumno: intentar crear decisión.
